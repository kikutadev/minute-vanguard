...
function arenaRoute(pathname) {
  ...
  const pets = pathname.match(/^\/v1\/games\/([^/]+)\/players\/([^/]+)\/arena\/pets$/u);
  if (pets) return { kind: 'arena-pets', gameId: decodeURIComponent(pets[1]), playerId: decodeURIComponent(pets[2]) };
...
async function getArenaRow(db, gameId, playerId) {
  return db.prepare(`
    SELECT game_id, player_id, display_name, job_id, rating, best_rating, season_key,
      ...
      next_attack_at_ms, barrier_until_ms, barrier_enabled, arena_weapon_id, arena_armor_id, arena_orb_id, arena_pet_primary, arena_pet_secondary, created_at_ms, updated_at_ms
...
async function handleArenaRegister(request, env, gameId, playerId) {
  ...
  if (existing === null) {
    ...
    `).bind(gameId, playerId, name, jobId, ARENA_START_RATING, ARENA_START_RATING, seasonKey, ARENA_DEFAULT_LOADOUT.weaponId, ARENA_DEFAULT_LOADOUT.armorId, ARENA_DEFAULT_LOADOUT.orbId, ARENA_DEFAULT_PETS.primary, ARENA_DEFAULT_PETS.secondary, nowMs, nowMs).run();
  } else {
...
async function handleArenaPets(request, env, gameId, playerId) {
  if (gameId !== GAME_ID) return json(request, { error: 'unknown-game' }, { status: 404 });
  const auth = await authenticateOwner(request, env.PUBLIC_PLAYER_DB, gameId, playerId);
  if (!auth.ok) return json(request, { error: auth.error }, { status: auth.status });
  const parsed = await readSmallJson(request, 512);
  if (parsed.error) return json(request, { error: parsed.error }, { status: parsed.error === 'payload-too-large' ? 413 : 400 });
  if (!isArenaPetLoadout(parsed.value)) return json(request, { error: 'invalid-arena-pets' }, { status: 400 });
  const nowMs = Date.now();
  const existing = await getArenaRow(env.PUBLIC_PLAYER_DB, gameId, playerId);
  if (existing === null) return json(request, { error: 'arena-not-joined' }, { status: 404 });
  await normalizeArenaSeason(env.PUBLIC_PLAYER_DB, existing, nowMs);
  const secondary = existing.job_id === 'job.tamer' ? parsed.value.secondary : 'none';
  await env.PUBLIC_PLAYER_DB.prepare(`
    UPDATE arena_players SET arena_pet_primary = ?, arena_pet_secondary = ?, updated_at_ms = ?
    WHERE game_id = ? AND player_id = ?
  `).bind(parsed.value.primary, secondary, nowMs, gameId, playerId).run();
  const updated = await getArenaRow(env.PUBLIC_PLAYER_DB, gameId, playerId);
  return json(request, { arena: arenaView(updated, await arenaRank(env.PUBLIC_PLAYER_DB, updated)) });
}
...
async function handleArenaBattle(request, env, gameId, playerId, matchType, challengedDefenderId = null) {
  ...
  const attackerPets = { primary: attacker.arena_pet_primary, secondary: attacker.arena_pet_secondary };
  const defenderPets = isBot
    ? ARENA_DEFAULT_PETS
    : { primary: opponent.arena_pet_primary, secondary: opponent.arena_pet_secondary };
  const seed = randomUint32();
  const simulation = simulateArenaBattle({ attackerJobId: attacker.job_id, defenderJobId, attackerLoadout, defenderLoadout, attackerPets, defenderPets, seed });
