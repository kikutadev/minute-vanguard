...
var e = Object.create, t = Object.defineProperty, n = Object.getOwnPropertyDescriptor, r = Object.getOwnPropertyNames, i = Object.getPrototypeOf, a = Object.prototype.hasOwnProperty, o = (e, t) => () => (t || (e((t = { exports: {} }).exports, t), e = null), t.exports), s = (e, i, o, s) => {
	...
	return new g().fromMantissaExponent(e, t);
}, p = function(e, t) {
	return new g().fromMantissaExponent_noNormalize(e, t);
};
function m(e, t, n, r) {
	var i = t.mul(n.pow(r));
	return g.floor(e.div(i).mul(n.sub(1)).add(1).log10() / n.log10());
}
function h(e, t, n, r) {
	return t.mul(n.pow(r)).mul(g.sub(1, n.pow(e))).div(g.sub(1, n));
}
var g = function() {
	function e(t) {
	...
	return Object.defineProperty(e.prototype, "m", {
		...
		return m(d(e), d(t), d(n), r);
	}, e.sumGeometricSeries = function(e, t, n, r) {
		return h(e, d(t), d(n), r);
...
}(), ee = p(1, l), te = p(1, -l), ne = d(Number.MAX_VALUE), re = d(Number.MIN_VALUE), _ = class e {
	#e;
	constructor(e) {
	...
	static zero() {
		return new e(g.fromNumber(0));
	}
	static one() {
		return new e(g.fromNumber(1));
	}
	static from(t) {
		return t instanceof e ? t : typeof t == "number" || typeof t == "string" ? new e(g.fromValue(t)) : e.deserialize(t);
	}
	static deserialize(t) {
		if (!Number.isFinite(t.mantissa) || !Number.isFinite(t.exponent)) throw RangeError("Serialized GameNumber must be finite.");
		return new e(g.fromMantissaExponent(t.mantissa, t.exponent));
	}
	add(t) {
	...
	static decimalOf(t) {
		return t instanceof e ? g.fromMantissaExponent(t.#e.m, t.#e.e) : typeof t == "number" || typeof t == "string" ? g.fromValue(t) : g.fromMantissaExponent(t.mantissa, t.exponent);
	}
}, ie = (e) => _.from(e);
//#endregion
...
function v(e, t) {
	switch (de(t), e.type) {
		case "linear": return _.from(e.base).add(_.from(e.step).multiply(t));
		case "polynomial": return ae(e.coefficients, t);
		case "geometric":
			if (!(e.ratio > 0) || !Number.isFinite(e.ratio)) throw RangeError("Geometric ratio must be finite and positive.");
			return _.from(e.base).multiply(_.from(e.ratio).pow(t));
		case "table": {
			let n = e.values[t];
		...
		case "piecewise": {
			let n = ce(e, t);
			return v(n.curve, t - n.startIndex);
		}
	}
}
function y(e, t, n) {
	if (de(t), fe(n), pe(t, n), n === 0) return _.zero();
	if (e.type === "linear") {
		let r = v(e, t), i = v(e, t + n - 1);
		return r.add(i).multiply(n).divide(2);
	}
	if (e.type === "polynomial") return oe(e.coefficients, t + n).subtract(oe(e.coefficients, t));
	if (e.type === "geometric") {
		if (!(e.ratio > 0) || !Number.isFinite(e.ratio)) throw RangeError("Geometric ratio must be finite and positive.");
		return e.ratio === 1 ? v(e, t).multiply(n) : v(e, t).multiply(_.from(e.ratio).pow(n).subtract(1)).divide(e.ratio - 1);
	}
	if (e.type === "piecewise") {
		ue(e.segments);
		let r = _.zero(), i = t, a = n;
		for (; a > 0;) {
			let t = le(e.segments, i), n = e.segments[t], o = e.segments[t + 1]?.startIndex, s = o === void 0 ? a : Math.min(a, o - i);
			r = r.add(y(n.curve, i - n.startIndex, s)), i += s, a -= s;
		}
		return r;
	}
	let r = _.zero();
	for (let i = t; i < t + n; i += 1) r = r.add(v(e, i));
	return r;
}
function ae(e, t) {
...
function oe(e, t) {
	if (de(t), e.length === 0) throw RangeError("Polynomial curve must contain at least one coefficient.");
	let n = [_.zero()], r = _.zero();
	for (let t = 0; t <= e.length - 1; t += 1) r = r.add(ae(e, t)), n.push(r);
	let i = n, a = _.zero();
	for (let e = 0; e < n.length; e += 1) a = a.add(i[0].multiply(se(t, e))), i = i.slice(1).map((e, t) => e.subtract(i[t]));
	return a;
}
function se(e, t) {
	if (t < 0 || t > e) return _.zero();
	let n = _.one();
	for (let r = 1; r <= t; r += 1) n = n.multiply(e - r + 1).divide(r);
	return n;
}
function ce(e, t) {
	return ue(e.segments), e.segments[le(e.segments, t)];
}
function le(e, t) {
...
function me(e) {
	ge(e.startIndex), he(e.curve);
	let t = _.from(e.balance);
	if (t.isNegative()) throw RangeError("Purchase balance must be non-negative.");
	let n = e.curve.type === "table" ? Math.max(0, e.curve.values.length - e.startIndex) : 2 ** 53 - 1 - e.startIndex, r = e.maxCount === void 0 ? n : Math.min(_e(e.maxCount), n);
	if (r === 0) return s(0);
	let i = (n) => y(e.curve, e.startIndex, n).compare(t) <= 0;
	if (!i(1)) return s(0);
	let a = 1, o = 1;
	for (; o < r;) {
	...
	return s(a);
	function s(n) {
		let i = y(e.curve, e.startIndex, n), a = n === r;
		return {
			count: n,
			totalCost: i,
			remainingBalance: t.subtract(i),
			nextUnitCost: a ? null : v(e.curve, e.startIndex + n),
			reachedLimit: a
		};
	}
}
function he(e) {
	switch (e.type) {
		case "linear":
			b(e.base, "Linear purchase curve base"), b(e.step, "Linear purchase curve step");
			return;
		case "polynomial":
			if (e.coefficients.length === 0) throw RangeError("Polynomial purchase curve must contain coefficients.");
			e.coefficients.forEach((e, t) => b(e, `Polynomial purchase curve coefficient ${t}`));
			return;
		case "geometric":
			if (b(e.base, "Geometric purchase curve base"), !(e.ratio > 0) || !Number.isFinite(e.ratio)) throw RangeError("Geometric purchase curve ratio must be positive and finite.");
			return;
		case "table":
			e.values.forEach((e, t) => b(e, `Table purchase curve value ${t}`));
			return;
		case "piecewise": {
			...
			for (let n of e.segments) {
				...
				t = n.startIndex, he(n.curve);
			}
		}
	}
}
function b(e, t) {
	if (_.from(e).isNegative()) throw RangeError(`${t} must be non-negative.`);
}
function ge(e) {
...
function ve(e, t) {
	...
	let n = /* @__PURE__ */ new Set(), r = e.filter((e) => {
		if (Se(e), n.has(e.id)) throw RangeError(`Duplicate attention candidate ID: ${e.id}`);
...
function ye(e) {
	...
	return e.filter((e) => {
		if (Ce(e), t.has(e.id)) throw RangeError(`Duplicate meaningful target ID: ${e.id}`);
...
function be(e, t) {
	let n = xe(t.urgency) - xe(e.urgency);
...
function Ee(e, t = 1) {
	...
	return i > 0 && i < Te.length ? `${(n.mantissa * 10 ** (r - i * 3)).toFixed(t).replace(/\.0+$/, "")}${Te[i]}` : `${n.mantissa.toFixed(t)}e${n.exponent}`;
}
//#endregion
...
function je(e, t, n) {
	...
	return !Me(r) || r.formatId !== "idle-game-kit-save-v1" || typeof r.gameId != "string" ? {
		accepted: !1,
		reason: "invalid-envelope"
	} : r.gameId === t ? n(r.state) ? typeof r.exportedAtMs != "number" || !Number.isFinite(r.exportedAtMs) ? {
		accepted: !1,
		reason: "invalid-envelope"
	} : {
		accepted: !0,
...
function S(e, t) {
	switch (e.type) {
		case "and": return e.conditions.every((e) => S(e, t));
		case "or": return e.conditions.some((e) => S(e, t));
		case "not": return !S(e.condition, t);
		...
		case "producer-level-at-least": return C(t.producerLevel, "producerLevel")(e.producerId) >= e.level;
		case "character-owned": return t.characterOwned(e.characterDefinitionId);
		case "character-level-at-least": return C(t.characterLevel, "characterLevel")(e.characterDefinitionId) >= e.level;
		case "activity-progress-at-least": return t.activityProgress(e.activityId) >= e.progress;
		case "activity-milestone-reached": return C(t.activityMilestoneReached, "activityMilestoneReached")(e.activityId, e.milestoneId);
		case "achievement-completed": return t.achievementCompleted(e.achievementId);
		case "prestige-count-at-least": return C(t.prestigeCount, "prestigeCount")(e.prestigeId) >= e.count;
		case "gacha-draw-count-at-least": return C(t.gachaDrawCount, "gachaDrawCount")(e.gachaId) >= e.count;
		case "calendar-streak-at-least": return C(t.calendarStreak, "calendarStreak")(e.calendarRewardId) >= e.count;
...
function Pe(e, t, n) {
	...
	return {
		...
		progress: t.progressMetric === void 0 ? null : r ? 1 : w(t.progressMetric, n)
	};
}
function w(e, t) {
	switch (e.type) {
		case "currency-balance": return Ie(t.currencyBalance(e.currencyId), e.target);
		case "lifetime-currency-earned": return Ie(t.lifetimeCurrencyEarned(e.currencyId), e.target);
...
function Fe(e) {
	let t = e.state, n = [];
	for (let r of e.definitions) t.achievements[r.id] !== !0 && S(r.condition, e.createConditionContext(t)) && (t = {
		...t,
		achievements: {
			...t.achievements,
			[r.id]: !0
...
function Ie(e, t) {
	let n = _.from(t);
	return n.compare(0) <= 0 || e.greaterThanOrEqual(n) ? 1 : e.compare(0) <= 0 ? 0 : Le(e.divide(n).toNumber());
}
function T(e, t) {
...
var D = (e, t) => _.deserialize(e[t] ?? _.zero().serialize());
function O(e, t, n) {
	n !== void 0 && ze(n, t.currencyId);
	let r = _.from(t.amount);
	if (r.isNegative()) return {
		accepted: !1,
		balances: e,
		reason: "invalid-amount"
	};
	let i = Re(r, n), a = D(e, t.currencyId);
	...
	if (n?.cap !== void 0) {
		let e = _.from(n.cap);
		a.compare(e) < 0 && o.compare(e) > 0 ? (o = e, s = e.subtract(a)) : a.compare(e) >= 0 && (o = a, s = _.zero());
	}
	return c(o, s);
	function c(n, r) {
		return {
...
function Re(e, t) {
	...
	let n = _.from(10).pow(t.precision), r = e.multiply(n);
...
function ze(e, t) {
	...
	if (e.cap !== void 0 && _.from(e.cap).isNegative()) throw RangeError("Currency cap must be non-negative.");
}
//#endregion
...
function Ve(e, t, n) {
	...
	for (let a of e) {
		...
		e.target === n && i.push(Be(e));
	}
	return i;
}
function He(e, t) {
	let n = t.filter((e) => e.operation === "flatAdd").reduce((e, t) => e.add(t.value), _.zero()), r = t.filter((e) => e.operation === "percentAdd").reduce((e, t) => e.add(t.value), _.zero()), i = t.filter((e) => e.operation === "multiply").reduce((e, t) => e.multiply(t.value), _.one()), a = _.from(e).add(n).multiply(_.one().add(r)).multiply(i), o = t.filter((e) => e.operation === "override").toSorted((e, t) => {
		let n = (t.overridePriority ?? 0) - (e.overridePriority ?? 0);
		return n === 0 ? e.id.localeCompare(t.id) : n;
	})[0];
	return o === void 0 ? a : _.from(o.value);
}
//#endregion
...
function Je(e, t, n = {}) {
	...
	for (let e of t) switch (e.type) {
		case "currency": {
			...
			let i = _.deserialize(r.statistics.lifetimeCurrencyEarned[e.currencyId] ?? _.zero().serialize());
			r = {
				...r,
		...
		case "producer": {
			...
			r = {
				...r,
				producers: Ue(r.producers, e.producerId, t + e.count)
			};
			break;
		}
		case "character":
			r = Ze(n.grantCharacter, "character")(r, e.characterDefinitionId);
			break;
		case "token":
			r = {
				...r,
				tokens: Ge(r.tokens, e.tokenId, e.count)
			};
			break;
		case "boost":
			r = Ze(n.activateBoost, "boost")(r, e.boostId);
			break;
		case "permanent-modifier":
			r = Ze(n.grantPermanentModifier, "permanent-modifier")(r, e.modifierId);
			break;
		...
		case "unlock":
			...
			break;
		case "composite": r = Je(r, e.rewards, n);
	}
	return r;
}
function Ye(e, t, n) {
	let r = _.deserialize(e.statistics.lifetimeCurrencySpent[t] ?? _.zero().serialize());
	return {
		...e,
		statistics: {
			...e.statistics,
...
function Qe(e) {
	let t = _.from(e.definition.baseAmount);
	...
	if (e.definition.eligibility !== void 0) {
		...
		if (!S(e.definition.eligibility, e.createConditionContext(e.state))) return {
			accepted: !1,
			state: e.state,
			events: [],
			reason: "ineligible"
	...
	let n = He(t, e.modifiers ?? []);
	if (n.isNegative()) throw RangeError("Active gain modifiers must not produce a negative reward.");
	let r = D(e.state.currencies, e.definition.currencyId), i = e.resolveCurrencyDefinition === void 0 ? {} : { resolveCurrencyDefinition: e.resolveCurrencyDefinition }, a = Je(e.state, [{
		type: "currency",
...
function $e(e) {
	if (A(e.definition), !(e.definition.eligibility === void 0 || S(e.definition.eligibility, at(e.createConditionContext)(e.state)))) return {
		eligible: !1,
		affordable: !0,
		canStart: !1,
		blockingCost: null
...
function et(e) {
	...
	for (let n of e.costs) {
		...
		let r = Ke(t.tokens, n.tokenId, n.count);
		if (!r.accepted) return {
			accepted: !1,
			state: e.state,
			blockingCost: n,
...
function nt(e, t, n) {
	if (A(e), !Number.isSafeInteger(t.maxSlots) || t.maxSlots <= 0) throw RangeError("Activity concurrency maxSlots must be a positive safe integer.");
...
function rt(e, t) {
	return (e.startCosts ?? []).filter((e) => e.type === "currency" && e.currencyId === t).reduce((e, t) => e.add(t.amount), _.zero());
}
function A(e, t) {
...
function ot(e) {
	if (A(e.definition, "continuous"), !Number.isSafeInteger(e.elapsedSec) || e.elapsedSec < 0) throw RangeError("elapsedSec must be a non-negative safe integer.");
	...
	for (; r > 0;) {
		...
		for (let [n, r] of Object.entries(o.production)) t = e.hooks.grantProduction(t, n, _.from(r).multiply(l));
		n = {
			...n,
...
function st(e, t, n) {
	if (A(t, "continuous"), !Number.isSafeInteger(n) || n < 0) throw RangeError("simTimeSec must be a non-negative safe integer.");
...
function lt(e) {
	if (j(e.definition), M(e.startSimTimeSec, "startSimTimeSec"), e.activity.status !== "available") return ht(e.state, e.activity, "not-available");
	let t = {
		...e.activity,
		status: "running",
	...
	return mt(e.state, t, [N(e.definition.id, "timedActivityStarted", e.startSimTimeSec, e.activity.completionCount + 1)]);
}
function ut(e) {
	if (j(e.definition), M(e.targetSimTimeSec, "targetSimTimeSec"), e.offlineElapsedSec !== void 0 && M(e.offlineElapsedSec, "offlineElapsedSec"), e.activity.status !== "running") return {
		state: e.state,
		activity: e.activity,
		events: []
	};
	...
	return {
		state: r,
		activity: pt(e.activity, e.definition, i),
		events: [N(e.definition.id, "timedActivityAutoResolved", t, i)]
	};
}
function dt(e, t, n) {
	return j(t), M(n, "simTimeSec"), e.status === "running" ? t.cancellationPolicy === "forbid" ? {
		accepted: !1,
		activity: e,
		events: [],
		reason: "cancellation-forbidden"
...
function ft(e) {
	if (j(e.definition), M(e.claimSimTimeSec, "claimSimTimeSec"), e.activity.status !== "completed-unclaimed") return ht(e.state, e.activity, "not-claimable");
	let t = e.hooks.grantRewards(e.state, e.definition.completionRewards), n = e.activity.completionCount + 1;
	return mt(t, pt(e.activity, e.definition, n), [N(e.definition.id, "timedActivityClaimed", e.claimSimTimeSec, n)]);
}
function pt(e, t, n) {
	return {
		...e,
...
function j(e) {
	if (A(e, "timed"), !Number.isSafeInteger(e.durationSec) || e.durationSec <= 0) throw RangeError("Timed Activity durationSec must be a positive safe integer.");
}
function M(e, t) {
...
function St(e, t, n, r) {
	...
	let i = xt(n, r, t.period), a = e.calendarRewardStates[t.id] ?? {
...
function Tt(e, t, n, r, i) {
	let a = St(e, t, n, r), o = e.calendarRewardStates[t.id] ?? {
		lastClaimedPeriodIndex: null,
		claimCount: 0,
		streakCount: 0
	};
...
var Et = (e, t) => Object.values(e).some((e) => e.definitionId === t);
function Dt(e, t) {
...
function Mt(e) {
	let t = e.cooldown ?? At(), n = jt(e.definition, t, e.simTimeSec);
	if (!n.ready) return {
		accepted: !1,
		cooldown: t,
...
function Nt(e) {
	Ft(e.cooldown), Lt(e.simTimeSec, "simTimeSec"), It(e.reductionSec, "reductionSec");
...
function Pt(e) {
	...
	It(e.durationSec, "Cooldown durationSec");
}
function Ft(e) {
	if (Lt(e.readyAtSimTimeSec, "Cooldown readyAtSimTimeSec"), !Number.isSafeInteger(e.useCount) || e.useCount < 0) throw RangeError("Cooldown useCount must be a non-negative safe integer.");
}
function It(e, t) {
	if (!Number.isSafeInteger(e) || e < 0) throw RangeError(`${t} must be a non-negative safe integer.`);
}
...
function Rt(e) {
	...
	for (let [r, i] of Object.entries(e.ids)) Bt(i, `ids.${r}`, t, n);
	...
	for (let [n, r] of (e.modifierDefinitions ?? []).entries()) {
		...
		d.has(r.id) && U(t, "duplicate-id", `${e}.id`, `Duplicate ModifierDefinition ID: ${r.id}`), d.add(r.id), H(l.permanentModifiers, r.id, `${e}.id`, "permanent modifier", t), Yt(r, e, t);
	}
	let f = /* @__PURE__ */ new Set();
	for (let [n, r] of (e.currencyDefinitions ?? []).entries()) {
		let e = `currencyDefinitions[${n}]`;
		f.has(r.id) && U(t, "duplicate-id", `${e}.id`, `Duplicate CurrencyDefinition ID: ${r.id}`), f.add(r.id), H(l.currencies, r.id, `${e}.id`, "currency", t), Vt(r, e, t);
	}
	...
	for (let [n, r] of (e.activeGains ?? []).entries()) {
		let e = `activeGains[${n}]`;
		H(l.currencies, r.currencyId, `${e}.currencyId`, "currency", t), Zt(r.baseAmount, `${e}.baseAmount`, t), r.eligibility !== void 0 && L(r.eligibility, `${e}.eligibility`, l, t);
	}
	...
	for (let [e, n] of i.entries()) {
		...
		Ht(n, "timed", r, l, t), (!Number.isSafeInteger(n.durationSec) || n.durationSec <= 0) && U(t, "invalid-value", `${r}.durationSec`, "Timed Activity durationSec must be a positive safe integer."), I(n.completionRewards, `${r}.completionRewards`, l, t);
	}
	let h = /* @__PURE__ */ new Map();
	for (let [n, r] of (e.boosts ?? []).entries()) {
		let e = `boosts[${n}]`;
		(!Number.isSafeInteger(r.durationSec) || r.durationSec <= 0) && U(t, "invalid-value", `${e}.durationSec`, "Boost durationSec must be a positive safe integer."), Xt(r.modifier, `${e}.modifier`, t), r.target.length === 0 && U(t, "invalid-value", `${e}.target`, "Boost target must not be empty."), r.stackingKey.length === 0 && U(t, "invalid-value", `${e}.stackingKey`, "Boost stackingKey must not be empty."), r.stackingPolicy === "replace-weaker" && (!Number.isFinite(r.stackingStrength) || r.stackingStrength <= 0) && U(t, "invalid-value", `${e}.stackingStrength`, "replace-weaker Boost stackingStrength must be positive and finite.");
		let i = h.get(r.stackingKey);
		i !== void 0 && i.policy !== r.stackingPolicy ? U(t, "invalid-value", `${e}.stackingPolicy`, `Boosts sharing stackingKey ${r.stackingKey} must use one stacking policy; ${i.path} uses ${i.policy}.`) : i === void 0 && h.set(r.stackingKey, {
	...
	for (let [n, r] of c.entries()) {
		...
		for (let [e, n] of (r.guaranteedSlots ?? []).entries()) {
			...
			Wt(n.id, a, s, t), r.allowedDrawCounts.includes(n.drawCount) || U(t, "invalid-value", `${a}.drawCount`, `Guaranteed slot drawCount is not allowed: ${n.drawCount}`), (!Number.isSafeInteger(n.drawIndex) || n.drawIndex < 0 || n.drawIndex >= n.drawCount) && U(t, "invalid-value", `${a}.drawIndex`, "Guaranteed slot drawIndex must be within its drawCount.");
			let l = `${n.drawCount}:${n.drawIndex}`;
			c.has(l) && U(t, "duplicate-id", a, `Duplicate guaranteed Gacha slot: ${l}`), c.add(l), Gt(n.poolEntryIds, o, `${a}.poolEntryIds`, t);
		}
		let u = /* @__PURE__ */ new Set();
		for (let [e, n] of (r.batchGuarantees ?? []).entries()) {
			let a = `${i}.batchGuarantees[${e}]`;
			Wt(n.id, a, s, t), r.allowedDrawCounts.includes(n.drawCount) || U(t, "invalid-value", `${a}.drawCount`, `Batch guarantee drawCount is not allowed: ${n.drawCount}`), !Number.isSafeInteger(n.batchSize) || n.batchSize <= 0 || n.batchSize > n.drawCount ? U(t, "invalid-value", `${a}.batchSize`, "Batch guarantee batchSize must be a positive safe integer <= drawCount.") : n.drawCount % n.batchSize !== 0 && U(t, "invalid-value", `${a}.batchSize`, "Batch guarantee batchSize must evenly divide drawCount."), u.has(n.drawCount) && U(t, "duplicate-id", a, `Only one batch guarantee may target drawCount ${n.drawCount}.`), u.add(n.drawCount), Gt(n.poolEntryIds, o, `${a}.poolEntryIds`, t);
		}
		if (r.pity !== void 0) {
			let e = `${i}.pity`;
			Wt(r.pity.id, e, s, t), (!Number.isSafeInteger(r.pity.threshold) || r.pity.threshold <= 0) && U(t, "invalid-value", `${e}.threshold`, "Gacha pity threshold must be a positive safe integer."), Gt(r.pity.poolEntryIds, o, `${e}.poolEntryIds`, t);
...
function Ht(e, t, n, r, i) {
	...
	for (let [t, a] of (e.startCosts ?? []).entries()) {
		...
		if (a.type === "currency") {
			...
			try {
				_.from(a.amount).isZero() && U(i, "invalid-value", `${e}.amount`, "Activity currency start cost must be positive.");
			} catch {}
			continue;
		}
		H(r.tokens, a.tokenId, `${e}.tokenId`, "token", i), Qt(a.count, `${e}.count`, "Activity token start cost", i);
	}
}
function F(e, t, n) {
	switch (e.type) {
	...
	for (let [i, a] of e.entries()) {
		...
		switch (a.type) {
			case "currency":
				...
				break;
			case "producer":
				H(n.producers, a.producerId, `${e}.producerId`, "producer", r), Qt(a.count, `${e}.count`, "Producer reward count", r);
				break;
			case "character":
				H(n.characters, a.characterDefinitionId, `${e}.characterDefinitionId`, "character", r);
				break;
			case "token":
				H(n.tokens, a.tokenId, `${e}.tokenId`, "token", r), Qt(a.count, `${e}.count`, "Token reward count", r);
				break;
			case "boost":
...
function Ut(e, t, n, r) {
	switch (e.type) {
		...
		case "lifetime-currency-earned":
			H(n.currencies, e.currencyId, `${t}.currencyId`, "currency", r), Zt(e.target, `${t}.target`, r);
			return;
		case "producer-count":
		case "producer-level":
...
function qt(e, t) {
	let n = new Map(e.map((e) => [e.id, Jt(e.condition)])), r = /* @__PURE__ */ new Set(), i = /* @__PURE__ */ new Set(), a = (e, o) => {
		if (!i.has(e)) {
			if (r.has(e)) {
				let n = o.indexOf(e);
...
function Jt(e) {
	switch (e.type) {
		...
		case "and":
		case "or": return e.conditions.flatMap(Jt);
		case "not": return Jt(e.condition);
		default: return [];
	}
}
function Yt(e, t, n) {
	e.target.length === 0 && U(n, "invalid-value", `${t}.target`, "Modifier target must not be empty."), e.source.length === 0 && U(n, "invalid-value", `${t}.source`, "Modifier source must not be empty."), Xt(e, t, n);
}
function Xt(e, t, n) {
...
function Zt(e, t, n) {
	try {
		_.from(e).compare(0) <= 0 && U(n, "invalid-value", t, "Value must be positive.");
	} catch {
...
function rn(e) {
	if (sn(e.definition), !e.definition.allowedDrawCounts.includes(e.drawCount)) return dn(e.state, "invalid-draw-count");
	let t = e.state.rngStreams[e.definition.rngStreamName];
	if (t === void 0) return dn(e.state, "missing-rng-stream");
	let n = _.from(e.definition.cost.amountPerDraw).multiply(e.drawCount), r = O(e.state.currencies, {
	...
	if (!r.accepted) return dn(e.state, "insufficient-currency");
	let i = {
		...e.state,
		currencies: r.balances
	};
	i = Ye(i, e.definition.cost.currencyId, r.appliedAmount);
	...
	for (let t = 0; t < e.drawCount; t += 1) {
		...
		let o = Math.floor(t / u.batchSize), d = an({
			definition: e.definition,
			drawCount: e.drawCount,
			drawIndex: t,
			stream: c,
...
function an(e) {
	let t = en(e.stream), n = e.definition.pity, r = n !== void 0 && e.pityMissCount + 1 >= n.threshold, i = e.definition.guaranteedSlots?.find((t) => t.drawCount === e.drawCount && t.drawIndex === e.drawIndex), a, o;
	...
	let s = on(o === void 0 ? e.definition.pool : e.definition.pool.filter((e) => o.includes(e.id)), t.value), c = n === void 0 ? e.pityMissCount : n.poolEntryIds.includes(s.id) ? 0 : e.pityMissCount + 1;
	return {
		draw: {
			entry: s,
			selectionRule: a,
...
function sn(e) {
	...
	if (_.from(e.cost.amountPerDraw).isNegative()) throw RangeError("Gacha cost must be non-negative.");
	...
	for (let a of e.guaranteedSlots ?? []) {
		if (cn(a.id, r), !n.has(a.drawCount)) throw RangeError(`Guaranteed slot drawCount is not allowed: ${a.drawCount}`);
		...
		i.add(e), ln(a.poolEntryIds, t, `Guaranteed slot ${a.id}`);
	}
	let a = /* @__PURE__ */ new Set();
	for (let i of e.batchGuarantees ?? []) {
		if (cn(i.id, r), !n.has(i.drawCount)) throw RangeError(`Batch guarantee drawCount is not allowed: ${i.drawCount}`);
		...
		a.add(i.drawCount), ln(i.poolEntryIds, t, `Batch guarantee ${i.id}`);
	}
	if (e.pity !== void 0) {
		if (cn(e.pity.id, r), !Number.isSafeInteger(e.pity.threshold) || e.pity.threshold <= 0) throw RangeError("Gacha pity threshold must be a positive safe integer.");
		ln(e.pity.poolEntryIds, t, `Pity ${e.pity.id}`);
...
function hn(e) {
	if (yn(e.loadoutDefinition), e.loadout.definitionId !== e.loadoutDefinition.id) return {
		accepted: !1,
		loadout: e.loadout,
...
function gn(e, t, n) {
	if (yn(e), t.definitionId !== e.id) return {
		accepted: !1,
		loadout: t,
		reason: "definition-mismatch"
	};
...
function bn(e) {
	...
	if (e.definition.eligibility !== void 0) {
		...
		if (!S(e.definition.eligibility, e.conditionContext)) return {
			available: !1,
			currentLevel: e.currentLevel,
			requestedCount: t,
			reason: "ineligible"
	...
	let i = y(e.definition.costCurve, e.currentLevel - 1, t);
	...
	return {
		...
		count: t,
		totalCost: i,
		currentStat: e.definition.statCurve === void 0 ? null : v(e.definition.statCurve, e.currentLevel - 1),
		targetStat: e.definition.statCurve === void 0 ? null : v(e.definition.statCurve, r - 1),
		crossedMilestones: a
	};
}
function xn(e) {
...
function wn(e, t, n, r) {
	...
	return {
		...
		progress: a ? 1 : Mn(i, t, r),
		points: Vn(t.points)
	};
}
function Tn(e, t, n, r) {
	K(r);
	let i = Pn(e, t, n, r), a = G(e, t.id, r), o = new Set(a?.claimedMilestoneIds ?? []);
	return {
		id: t.id,
		periodIndex: r,
		points: i,
...
function En(e) {
	if (K(e.periodIndex), In(e.state, e.setDefinition, e.missionDefinitions, e.periodIndex)) return {
		accepted: !1,
		state: e.state,
		events: [],
		reason: "clock-rollback"
	};
	let t = Fn(e.setDefinition, e.missionDefinitions), n = e.state, r = [];
	for (let i of e.setDefinition.missionIds) {
		let a = t.get(i);
		if (a === void 0 || a.objective.type !== "condition" || W(n, a.id, e.periodIndex)?.completed === !0 || !S(a.objective.condition, e.createConditionContext(n))) continue;
		let o = An(n, a, e.periodIndex, e.grantRewards);
		n = o.state, r.push(...o.events);
	}
	let i = jn(n, e.setDefinition, e.missionDefinitions, e.periodIndex, e.grantRewards);
	return {
		accepted: !0,
		state: i.state,
		events: [...r, ...i.events]
...
function Dn(e) {
	...
	let t = Nn(e.updates), n = Fn(e.setDefinition, e.missionDefinitions), r = e.state, i = [];
	for (let a of e.setDefinition.missionIds) {
		...
		let l = c?.progress === void 0 ? _.zero() : _.from(c.progress), u = _.from(o.objective.target), d = l.add(s);
		if (r = Ln(r, o.id, {
			...
			let t = An(r, o, e.periodIndex, e.grantRewards, d.serialize());
			r = t.state, i.push(...t.events);
		}
	}
	let a = jn(r, e.setDefinition, e.missionDefinitions, e.periodIndex, e.grantRewards);
	return {
		accepted: !0,
		state: a.state,
		events: [...i, ...a.events]
...
function On(e, t, n, r) {
	...
	return o = Ln(o, t.id, {
		...
		state: o,
		rewards: t.rewards,
		events: [zn(o, "missionClaimed", t.id, n)]
	};
}
function kn(e, t, n, r, i, a) {
	...
	if (Pn(e, t, n, i) < s.pointsRequired) return {
		accepted: !1,
		state: e,
		events: [],
		reason: "not-reached"
	...
	return l = Rn(l, t.id, {
		...
		state: l,
		rewards: s.rewards,
		events: [Bn(l, t.id, s.id, i)]
	};
}
function An(e, t, n, r, i) {
	let a = (t.claimPolicy ?? "manual") === "auto", o = Ln(e, t.id, {
		periodIndex: n,
		completed: !0,
		claimed: a,
		...i === void 0 ? {} : { progress: i }
	}), s = [zn(o, "missionCompleted", t.id, n)];
	return a && (o = r(o, t.rewards), s.push(zn(o, "missionClaimed", t.id, n))), {
		state: o,
		events: s
	};
}
function jn(e, t, n, r, i) {
	let a = Pn(e, t, n, r), o = G(e, t.id, r), s = new Set(o?.claimedMilestoneIds ?? []), c = e, l = [];
	for (let e of t.pointMilestones ?? []) (e.claimPolicy ?? "manual") === "auto" && (s.has(e.id) || a < e.pointsRequired || (c = i(c, e.rewards), s.add(e.id), c = Rn(c, t.id, {
		periodIndex: r,
		claimedMilestoneIds: [...s]
	}), l.push(Bn(c, t.id, e.id, r))));
	return {
		state: c,
		events: l
	};
...
function Mn(e, t, n) {
	if (t.objective.type === "condition") return t.objective.progressMetric === void 0 ? null : w(t.objective.progressMetric, n);
	let r = _.from(t.objective.target);
	if (r.compare(0) <= 0) return 1;
	let i = e?.progress === void 0 ? _.zero() : _.from(e.progress);
...
function Pn(e, t, n, r) {
	...
	return t.missionIds.reduce((t, n) => {
		let a = i.get(n);
		return a === void 0 ? t : W(e, n, r)?.completed === !0 ? t + Vn(a.points) : t;
	}, 0);
}
function Fn(e, t) {
...
function Hn(e) {
	...
	return {
		opportunity: t,
		events: [Y(t, "opportunityOffered", e.offeredAtSimTimeSec)]
	};
}
function Un(e) {
	q(e.definition), qn(e.opportunity, e.definition), J(e.targetSimTimeSec, "targetSimTimeSec");
	...
	return {
		opportunity: i,
		events: [Y(i, "opportunityExpired", n)]
	};
}
function Wn(e) {
	if (q(e.definition), qn(e.opportunity, e.definition), J(e.acceptedAtSimTimeSec, "acceptedAtSimTimeSec"), e.opportunity.status !== "open") return X(e.opportunity, "not-open");
	...
	return Jn(t, Y(t, "opportunityAccepted", e.acceptedAtSimTimeSec));
}
function Gn(e) {
	if (q(e.definition), qn(e.opportunity, e.definition), J(e.dismissedAtSimTimeSec, "dismissedAtSimTimeSec"), e.opportunity.status !== "open") return X(e.opportunity, "not-open");
	...
	return Jn(t, Y(t, "opportunityDismissed", e.dismissedAtSimTimeSec));
}
function Kn(e, t) {
...
function Xn(e, t, n) {
	return {
		eligible: S(t.eligibility, n(e)),
		nextCount: (e.prestigeStates[t.id]?.count ?? 0) + 1,
		rewards: t.rewards(e),
		resetImpact: Yn(e, t.resetPolicy)
	};
}
function Zn(e, t, n) {
	let r = Xn(e, t, n.createConditionContext);
	if (!r.eligible) return {
		accepted: !1,
		state: e,
		reason: "not-eligible"
	};
	let i = Qn(e, t.resetPolicy);
...
function $n(e) {
	if (rr(e.definition), ir(e.simTimeSec, "simTimeSec"), !Number.isSafeInteger(e.dailyPeriodIndex)) throw RangeError("dailyPeriodIndex must be a safe integer.");
	...
	if (e.definition.eligibility !== void 0) {
		...
		if (!S(e.definition.eligibility, e.createConditionContext(e.state))) return $(!1, "ineligible", t, e);
	}
	if (nr(t, e.definition, e.simTimeSec) > 0) return $(!1, "cooldown", t, e);
...
function er(e) {
	let t = e.offerStates[e.definition.id] ?? tr(), n = $n({
		state: e.state,
		offerState: t,
		definition: e.definition,
		simTimeSec: e.simTimeSec,
...
function $(e, t, n, r) {
	...
	return {
		...
		cooldownRemainingSec: nr(n, r.definition, r.simTimeSec),
...
//#endregion
export { Ne as ApplicationStore, _ as GameNumber, x as SAVE_FORMAT_ID, Wn as acceptOpportunity, w as achievementProgressRatio, gt as activateTemporaryBoost, _t as activeBoostModifiers, tt as activityAdvancesOffline, rt as activityStartCurrencyCost, Dt as addCharacter, fn as addItemInstance, ot as advanceContinuousActivity, Un as advanceOpportunity, ut as advanceTimedActivity, et as applyActivityStartCosts, O as applyCurrencyTransaction, He as applyModifiers, Je as applyRewards, zt as assertValidDefinitionBundle, xt as calendarPeriodIndex, dt as cancelTimedActivity, Ct as claimCalendarReward, On as claimMission, kn as claimMissionPointMilestone, ft as claimTimedActivity, Mt as consumeCooldown, At as createCooldownState, mn as createLoadoutState, Hn as createOpportunityState, tn as createRngStreams, ct as createTimedActivityState, Xe as currencyBalance, y as curveIntervalSum, v as curveValueAt, ke as dismissCurrentPresentation, Gn as dismissOpportunity, rn as drawGacha, tr as emptyRewardedOfferState, Oe as enqueuePresentationItems, hn as equipItem, Fe as evaluateAchievements, S as evaluateCondition, En as evaluateMissions, Qe as executeActiveGain, Zn as executePrestige, Ee as formatGameNumber, ie as gameNumber, er as grantRewardedOffer, Ge as grantToken, kt as incrementCharacterLimitBreak, me as maxAffordableCurvePurchase, Be as modifierFromDefinition, vt as nextBoostExpiry, en as nextRandom, Kn as opportunityRemainingSec, Et as ownsCharacterDefinition, je as parseSaveImport, on as pickWeightedEntry, nt as previewActivityConcurrency, $e as previewActivityStart, St as previewCalendarReward, jt as previewCooldown, bn as previewLevelUp, Xn as previewPrestige, Yn as previewPrestigeResetImpact, $n as previewRewardedOffer, yt as pruneExpiredBoosts, D as readCurrency, k as readToken, Ye as recordCurrencySpend, Dn as recordMissionProgress, Nt as reduceCooldown, pn as removeItemInstance, wt as resolveAutomaticCalendarRewards, Ve as resolveModifierDefinitions, De as resolveOfflineElapsed, Pe as selectAchievementStatus, ve as selectAttentionSummary, Tn as selectMissionSetStatus, wn as selectMissionStatus, ye as selectNextMeaningfulTarget, ar as selectTitleStatus, or as selectVisibleTitles, Ae as serializeSave, Ot as setCharacterLevel, We as setProducerLevel, Ue as setProducerOwnedCount, Ke as spendToken, lt as startTimedActivity, st as stopContinuousActivity, gn as unequipItem, A as validateCommonActivityDefinition, Rt as validateDefinitionBundle, q as validateOpportunityDefinition };
