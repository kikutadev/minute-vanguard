...
Arena now includes server-owned free weapon/armor/orb loadouts, normalized Arena pets, ranking-targeted challenges, immutable weekly season archives, carry-forward season reward receipts claimed from Menu → Presents, an inspectable ten-tier reward ladder, promotion callouts in battle results, and Hall of Fame history. Server-attested tier reaches unlock ten persistent Arena achievement titles, and a claimed champion receipt unlocks the original weekly-champion title. Reward receipts are applied locally through the Kit external-grant idempotency contract and ACKed only after the save succeeds. Exact tier-by-tier reward amounts are Minute Vanguard-owned because the current public reference exposes only Gold + Gem, a 3–120 Gem range, Master crest, champion bonus and carry-forward behavior. Random-match Gold transfer remains deferred until Gold itself has trusted server authority. Shared Raid / wanted events, chat and payments remain separate future authority boundaries. API failure never blocks solo play.
