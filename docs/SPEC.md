...
Arena-specific Weapon / Armor / Orb selection is server-owned, free, and shared by attack and defense. Leaderboard names expose this same server-owned loadout and normalized combat stats, while active defense barriers are surfaced before challenge submission. Weekly season results are finalized into immutable D1 archives before per-player reset, with carry-forward reward receipts claimed from Menu → Presents and a public Hall of Fame. The Arena season card expands into all ten tiers and product-owned reward amounts, while the Worker returns a promotion payload when a battle crosses a tier threshold so both random and named-challenge result screens can show the same promotion reward preview. Receipt application uses the Kit external-grant ID to prevent duplicate local Gold/Gem grants, and server ACK happens only after IndexedDB save. Exact per-tier reward amounts and champion bonus amounts are Minute Vanguard-owned because the current reference does not publish the live table. The remaining major Arena economy gap is random-match Gold transfer; local save values are not trusted for competitive outcomes.
